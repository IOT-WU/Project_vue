<template>
  <div>
    <div style="text-align: left;">
      ...
      <el-input size="small" style="width: 200px;" v-model="name"></el-input>&emsp;
      <el-button size="small" type="danger" plain @click="show">查询</el-button>
    </div>
    <el-table :data="ReservationProcurement" style="width: 100%">
      <el-table-column prop="goods_Name" label="用品名称" width="180" />
      <el-table-column prop="goods_Specifications" label="规格" width="180" />
...
<script>
  export default {
    ...
    methods: {
      ...
      show() {
        this.$axios({
          url: "http://localhost:7438/api/GetProcurementDetails?Name=" + this.name,
          method: "get"
        }).then((res) => {
